  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="inspector" role="dialog" aria-modal="true" aria-label="Audited page details" onMouseDown={event => event.stopPropagation()}>
      ...
      <div className="detail-grid">
        <div><span>Status</span><strong>{page.statusCode ?? '—'}</strong></div><div><span>Latency</span><strong>{(page.responseTimeMs ?? page.responseTime) ? `${page.responseTimeMs ?? page.responseTime}ms` : '—'}</strong></div>
...
export default function App() {
  ...
  const [explorerView, setExplorerView] = useState<'pages' | 'links' | 'resources' | 'issues'>('pages');
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="card explorer">
        <div className="explorer-head"><div><p className="eyebrow">{explorerView === 'pages' ? 'Audited pages' : explorerView === 'links' ? 'Discovered links & anchors' : explorerView === 'resources' ? 'Resources & assets' : 'SEO issues'}</p><h2>{explorerView === 'pages' ? `${crawler.pages.length.toLocaleString()} page${crawler.pages.length === 1 ? '' : 's'} collected` : explorerView === 'links' ? `${crawler.links.length.toLocaleString()} link${crawler.links.length === 1 ? '' : 's'} collected` : explorerView === 'resources' ? 'Embedded resource inventory' : `${issueCount.toLocaleString()} issue${issueCount === 1 ? '' : 's'} identified`}</h2></div><div className="export-wrap"><button className="secondary" onClick={() => setExportOpen(open => !open)}>Export ▾</button>{exportOpen && <div className="export-menu">{[
