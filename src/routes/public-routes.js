...
export function registerPublicRoutes(app, { publicDir, publicAppUrl, requireDashboardAccess, staticMiddleware, crawlStorage, hashAuditorPassword }) {
  ...
  app.post('/api/public/signup', async (req, res) => {
    ...
    if (!process.env.RESEND_API_KEY || !process.env.EMAIL_FROM) return res.status(503).json({ error: 'Sign-up email is not configured yet.' });
    try {
      ...
      const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: process.env.EMAIL_FROM, to: [email], subject: 'Verify your CrawlLoom email', text: `Hi ${name},\n\nVerify your CrawlLoom email and create your auditor password here:\n${verifyUrl}\n\nThis link expires in 24 hours.` }) });
